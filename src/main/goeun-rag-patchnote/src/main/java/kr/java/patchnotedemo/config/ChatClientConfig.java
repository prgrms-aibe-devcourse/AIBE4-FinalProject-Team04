...
public class ChatClientConfig {
    ...
    //
    // OpenAiChatOptions.builder().model(openAIchatModelName).temperature(0.0).build();
    //
    //        return OpenAiChatModel.builder().openAiApi(geminiApi).defaultOptions(options).build();
    //    }

    @Bean
    @Qualifier("customOllamaChatModel")
    ...
    @Bean
    public ChatClient chatClient(@Qualifier("customOllamaChatModel") ChatModel chatModel) {
